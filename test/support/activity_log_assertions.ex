defmodule PhoenixKitManufacturing.ActivityLogAssertions do
  @moduledoc """
  Helpers for asserting activity log entries landed with the right action,
  module, and metadata shape.

  Imported into `PhoenixKitManufacturing.DataCase` and
  `PhoenixKitManufacturing.LiveCase` so every DB-backed test can reach them.
  """
  ...
  @doc """
  Asserts that exactly one activity row exists for `action`, with the given
  match criteria. Returns the matched row so further assertions can be made
  on it.
  ...
  def assert_activity_logged(action, opts \\ []) do
    ...
    matching = Enum.filter(rows, fn row -> matches_opts?(row, opts) end)
