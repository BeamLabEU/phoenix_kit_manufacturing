defmodule PhoenixKitManufacturing.Test.Layouts do
  @moduledoc """
  Minimal layouts for the LiveView test endpoint. Real layouts live in the
  host app and the phoenix_kit core — these just wrap LiveView content in an
  HTML shell so Phoenix.LiveViewTest can render it.

  `app/1` renders flashes (info / error / warning) with stable IDs so flash
  assertions in tests can match on `flash-info` / `flash-error` /
  ...
  # layouts module if a LiveView raises during mount. Forward everything to
  # a single generic template so tests get a readable error instead of a
